...
describe("tests for registration and logon (SPA)", function () {
  it("should get the logon page", async () => {
    const { expect, request } = await get_chai();
    const req = request.execute(app).get("/sessions/logon").send();
    const res = await req;
  ...
  it("should load SPA home after logon", async () => {
    const { expect, request } = await get_chai();
    const req = request
      .execute(app)
      .get("/")
    ...
    expect(res).to.have.status(200);
    expect(res).to.have.property("text");
    expect(res.text).to.include("Welcome to CoderPlanet-X!");
  });
